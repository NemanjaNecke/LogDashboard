...
class EVTXDatabaseManager:
        ...
        try:
            ...
            cursor.execute(f"""
                CREATE TABLE IF NOT EXISTS {table_name} (
                    ...
                    raw_xml TEXT,
                    time_taken REAL,  -- NEW COLUMN for durations
        ...
        try:
            ...
            for log in logs:
                data_batch.append((
                    ...
                    log.get("raw_xml"),
                    log.get("time_taken", None),  # default None if not found
                ))
    ...
    def get_all_timestamps(self, table_name="evtx_logs"):
        """
        Example method if you want both timestamp_epoch and time_taken
        so you can pass them to the timeline as (timestamp, time_taken).
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute(f"""
                SELECT timestamp_epoch, time_taken
                FROM {table_name}
                WHERE timestamp_epoch IS NOT NULL
                  -- Optionally filter out Channel='XML-Imported' or something if needed
            """)
            rows = cursor.fetchall()
            return [(float(row[0]), row[1] if row[1] else 0.0) for row in rows]
        except sqlite3.Error as e:
            self.logger.error(f"SQLite error in get_all_timestamps_with_duration: {e}")
            return []
        except Exception as e:
            self.logger.error(f"Unexpected error in get_all_timestamps_with_duration: {e}")
            return []

...
class DatabaseManager:
    ...
    def get_all_timestamps(self, table_name="iis_logs"):
        """
        Retrieves all 'combined_ts' timestamps along with their corresponding time_taken values
        from the specified table, packaging each as a tuple: (timestamp, time_taken_as_string).
        """
        self.logger.debug(f"Retrieving all timestamps with time_taken from table={table_name}")
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute(f"SELECT combined_ts, time_taken FROM {table_name} WHERE combined_ts IS NOT NULL")
            rows = cursor.fetchall()
            events = []
            for row in rows:
                if row[0] is not None:
                    ts = float(row[0])
                    # Convert time_taken to a string without any extra label.
                    time_taken = row[1]
                    time_taken_str = str(time_taken) if time_taken is not None else ""
                    events.append((ts, time_taken_str))
            conn.close()
            self.logger.info(f"Retrieved {len(events)} events (timestamp, time_taken) from {table_name}")
            
            # Log a few sample events for verification
            sample_events = events[:10]
            for i, ev in enumerate(sample_events):
                self.logger.info(f"Sample event {i}: timestamp={ev[0]}, time_taken={ev[1]}")

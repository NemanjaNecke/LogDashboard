...
from logdashboard.services.sql_workers.db_managers.EVTX.workers_evtx import EVTXInsertWorker, TimestampLoader
from logdashboard.services.sql_workers.db_managers.EVTX.stats_loader import FieldStatsLoader, StatsLoader
from logdashboard.services.sql_workers.db_managers.EVTX.db_manager_evtx import EVTXDatabaseManager

# Table model & details dialog & stats panel
from logdashboard.ui.components.display_logs.EVTX.log_table_view import EVTXTableModel
from logdashboard.ui.components.display_logs.EVTX.event_details_dialog import EventDetailsDialog
from logdashboard.ui.components.display_logs.EVTX.stats_panel import StatsPanel
...
class EVTXDock(QDockWidget):
    ...
    def __init__(self, file_path="", db_path="", parent=None):
        ...
        self.stats_container = QWidget()  # save as instance variable
        ...
        self.stats_container.setFixedHeight(150)
        ...
        # New: Toggle Analytics button
        self.toggle_analytics_btn_evtx = QPushButton("Hide Analytics")
        self.toggle_analytics_btn_evtx.clicked.connect(self.toggleAnalytics)
        options_layout.addWidget(self.toggle_analytics_btn_evtx)
    ...
    def toggleAnalytics(self):
        # Toggle the visibility of the stats_container
        currently_visible = self.stats_container.isVisible()
        self.stats_container.setVisible(not currently_visible)
        if currently_visible:
            self.toggle_analytics_btn_evtx.setText("Show Analytics")
        else:
            self.toggle_analytics_btn_evtx.setText("Hide Analytics")
    ...
    @pyqtSlot(object)
    def onTimestampsLoaded(self, timestamp_pairs):
        """
        timestamp_pairs is a list of (timestamp_epoch, time_taken).
        We'll wrap them exactly as the timeline expects.
        """
        self.logger.debug(f"Loaded {len(timestamp_pairs)} (ts, duration) pairs.")
        timeline_dock = self.findTimelineDock()
        if timeline_dock:
            # Add them under source name e.g. "evtx_duration"
            timeline_dock.addTimestamps("evtx_duration", timestamp_pairs)
        else:

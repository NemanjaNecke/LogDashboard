...
from logdashboard.ui.components.display_logs.GENERIC.analytics_gadget import AnalyticsGadget
from logdashboard.services.sql_workers.db_managers.GENERIC.workers_generic import GenericLogToSQLiteWorker
from logdashboard.services.sql_workers.db_managers.GENERIC.db_manager_generic import GenericDBManager
from logdashboard.ui.components.display_logs.GENERIC.log_table_view import GenericTableModel
...
class GenericDock(QDockWidget):
    ...
    def populateTextPreview(self):
        ...
        self.text_viewer.setPlainText(all_content)
    ...
    def closeEvent(self, event):
        ...
        # If a temporary DB was created, delete it when the dock is closed
        if self.db_path and os.path.exists(self.db_path):
            try:
                os.remove(self.db_path)
                self.logger.info(f"Temporary DB {self.db_path} deleted.")
            except Exception as e:
                self.logger.warning(f"Failed to delete temporary DB {self.db_path}: {e}")
...
# Import other docks
from logdashboard.ui.components.timeline.dock_timeline_plotly import TimelineDock
from logdashboard.ui.components.display_logs.IIS.dock_iis import IISDock
from logdashboard.ui.components.display_logs.EVTX.dock_evtx import EVTXDock
from logdashboard.ui.components.display_logs.GENERIC.dock_generic import GenericDock
from logdashboard.services.logging.dock_log import LogDock
from logdashboard.ui.components.db_load.dock_db_manager import DBManagerDock
from logdashboard.ui.components.charts.dock_3d import ThreeDDock
from logdashboard.services.analyze.IIS.iis_analyze_worker import AnalyzerWorker
from logdashboard.ui.components.display_logs.IIS.stats.analyze_dialog import AnalysisDialog  # Import AnalysisDialog
from logdashboard.ui.components.display_logs.IIS.stats.all_stats_panel import AllStatsPanel  # Import AllStatsPanel
from logdashboard.ui.components.display_logs.IIS.table.sheet_selection_dialog import SheetSelectionDialog  # Import SheetSelectionDialog
from logdashboard.ui.components.display_logs.IIS.failed_request.failed_request_trace_dialog import FrebViewerDock

class MainWindow(QMainWindow):

    def __init__(self, selected_modules=None):
        """
        :param selected_modules: A list or set of module names for optional features:
                                e.g. ["IIS", "EVTX", "GENERIC"].
        Timeline, DB Manager, and Log Dock will always be available.
        """
        super().__init__()
        self.setWindowTitle("Log Dashboard")
        self.resize(1600, 1900)

        # If user didn’t pass anything, default to empty
        ...
        # Prepare references for docks, worker, etc.
        self.iis_dock = None
        self.evtx_dock = None
        self.generic_dock = None
        self.timeline_dock = None
        self.log_dock = None
        self.db_manager_dock = None
        self.worker = None
        self.progress_dialog = None

        self.create_docks()  # Always create Timeline, Log Dock, DB Manager
        self.createMenuBar()  # Conditionally create menus for IIS, EVTX, GENERIC

    def create_docks(self):
        """
        Create Timeline, LogDock, and DBManager by default (always).
        """
        # 1. Timeline Dock (always)
        ...
        # 2. Log Dock (always, but hidden by default if you prefer)
        ...
        # 3. DB Manager Dock (always, but hidden by default)
    ...
    def createMenuBar(self):
        ...
        # Possibly add a "Show Log Dock" item here
        show_log_dock_action = QAction(QIcon.fromTheme("view-list-details"), "Show Log Dock", self)
        show_log_dock_action.triggered.connect(lambda: self.toggleDock(self.log_dock, True))
        view_menu.addAction(show_log_dock_action)

        show_db_manager_action = QAction(QIcon.fromTheme("database"), "Show DB Manager", self)
        show_db_manager_action.triggered.connect(lambda: self.toggleDock(self.db_manager_dock, True))
        view_menu.addAction(show_db_manager_action)

        ################################
        # IIS Menu (only if "IIS" selected)
        ################################
        if "IIS" in self.selected_modules:
        ...
        ################################
        # EVTX Menu (only if "EVTX" selected)
        ################################
        if "EVTX" in self.selected_modules:
        ...
        ################################
        # GENERIC Menu (only if "GENERIC" selected)
        ################################
        if "GENERIC" in self.selected_modules:
            ...
            # If you want a stored Generic DB option:
            # open_stored_generic_action = QAction(QIcon.fromTheme("document-open"), "Open Stored Generic Log...", self)
            # open_stored_generic_action.triggered.connect(self.openGenericDatabaseDialog)
            # generic_menu.addAction(open_stored_generic_action)

        ################################
        # HELP Menu (always or optional)
        ################################
        help_menu = menubar.addMenu("Help")
        # (Add help/documentation actions here if you like)
    ...
    def openFrebTrace(self):
        ...
        dock = FrebViewerDock(parent=self)
        self.addDockWidget(Qt.RightDockWidgetArea, dock)
